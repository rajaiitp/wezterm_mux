...
- role-based module profiles with named `single`, `tabs`, `columns`, `rows`,
...
mux owns all tab creation, split sizing, and pane geometry. The provided dotfile
configuration opens `pi --continue` and `nvim .` in separate tabs using the
`tabs` layout; tuicr is available as a separate app toggle unless explicitly
